...
export class ChatListener extends EventEmitter {
  private isActive: boolean = false;
  private ws: WebSocket | null = null;
  private reconnectTimeout: NodeJS.Timeout | null = null;
  private readonly TIKFINITY_WS_URL = 'ws://localhost:21213/';
  private readonly SOUNDS_DIR: string;
  ...
  private connectToTikFinity(): void {
    if (!this.isActive) return;

    try {
      this.ws = new WebSocket(this.TIKFINITY_WS_URL);
  ...
  private handleTikFinityEvent(event: { event: string; data: any }): void {
    if (!this.isActive) return;
  ...
  private handleChatEvent(data: any): void {
  ...
  private handleGiftEvent(data: any): void {
    const giftMessage: TikTokMessage = {
      username: data.uniqueId || data.nickname || 'Unknown',
  ...
  private handleFollowEvent(data: any): void {
    const followMessage: TikTokMessage = {
      username: data.uniqueId || data.nickname || 'Unknown',
  ...
  handleMessage(message: TikTokMessage): void {
    if (!this.isActive) return;
